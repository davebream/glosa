...
/** What `detectSession` extracts from a raw session/event payload (R7 verbatim shape). `workspace`
 ...
 * `transcript_path` is optional (not every provider has one); `source` is the registration
 * source label (`"monitor"|"codex-app-server"|"mcp"|"cli"`, A2 §F08). */
...
/** R7 verbatim — the two delivery mechanisms a provider may offer (R4's table: push into an
 * idle session over the provider's own transport, pull on demand over MCP). **Evaluated per
 * session at registration**, never provider-wide: Claude's `push` is true only while that session
 * has a connected plugin monitor; Codex's only while that thread owns a live app-server
 * attachment. A provider that can't do one just reports `false` for it; `deliver()` is the only
 * thing that needs to know how to fall back through whichever subset is `true`. */
 ...
 * boundary), `transport_accepted` (the transport layer itself ack'd it — the monitor or app-server stream
 * took the entry), `presented` (actually reached agent context — the agent's own
 * `glosa_delivery_ack`, or the MCP pull route surfacing it), `failed` (a rung that was
...
export type DeliverableEntry =
  ...
  | (PresentationBase & { kind: "external_edit"; detail: Record<string, unknown> })
...
export interface AgentProvider {
  ...
  /** Lease/heartbeat only — NEVER `kill(pid, 0)` (A2 §F08: a session payload has no documented PID). */
