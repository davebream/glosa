...
// Real `__daemon` subprocess suites need headroom past Bun's default 5s: beforeEach spawn +
// handshake alone is usually fast, but under full-suite load (and double-spawn cases like
// paired:false) the default budget flakes. Same precedent as lifecycle.test.ts (10–25s).
...
describe("daemon HTTP pipeline — real subprocess", () => {
  ...
  /** Same as `test` but with the subprocess budget (Bun default 5s is too tight here). */
  function it(name: string, fn: () => Promise<void> | void): void {
    test(name, fn, SUBPROCESS_TEST_TIMEOUT_MS);
  }

  it("Host mismatch → 400, no body [A3 §5 #7]", async () => {
  ...
  it("handshake: no Origin → 200 + superset body (P1.2 fields + A1 §5.1 fields)", async () => {
    ...
    expect(body.contract_version).toBe("1.12");
  ...
  it("handshake with foreign Origin → 403", async () => {
  ...
  it("handshake reflects paired:false when no token file exists", async () => {
    const home2 = freshHome();
    const port2 = randomPort();
    const proc2 = spawnDaemon(home2, port2, { GLOSA_CLASSF_PORT: String(port2 + 1) });
    try {
      const hs = await waitForHandshake(port2, 15_000, proc2);
      expect(hs, `daemon handshake failed (exitCode=${proc2.exitCode})`).not.toBeNull();
  ...
  it("authed GET /api/workspaces with no Bearer → 401", async () => {
    const res = await fetch(apiUrl("/api/workspaces"));
    expect(res.status).toBe(401);
  });

  it("authed GET /api/workspaces with valid Bearer + no Origin → 200 []", async () => {
  ...
  it("state-changing POST with valid Bearer but missing Origin → 403", async () => {
  ...
  it("state-changing POST with valid Bearer + foreign Origin → 403", async () => {
    const res = await fetch(apiUrl("/w/some-slug/session-binding"), {
      method: "POST",
      headers: {
        Authorization: `Bearer ${TOKEN}`,
  ...
  it("state-changing POST with Sec-Fetch-Site: cross-site → 403 even with self Origin + valid Bearer", async () => {
    const res = await fetch(apiUrl("/w/some-slug/session-binding"), {
      method: "POST",
      headers: {
        Authorization: `Bearer ${TOKEN}`,
  ...
  it("state-changing POST with self Origin + valid Bearer → passes auth, then 404 (empty registry) [A3 §5 #7]", async () => {
    const res = await fetch(apiUrl("/w/some-slug/session-binding"), {
      method: "POST",
      headers: {
        Authorization: `Bearer ${TOKEN}`,
  ...
  it("X-Contract-Version is not checked on the handshake route", async () => {
    const res = await fetch(apiUrl("/api/handshake"), { headers: { "X-Contract-Version": "99.0" } });
    expect(res.status).toBe(200);
  });

  it("body > 1 MiB on the POST → 413", async () => {
  ...
  it("SPA-origin CSP headers attached to API responses", async () => {
    const res = await fetch(apiUrl("/api/handshake"));
  ...
  it("foreign Origin + unknown route → 403, not 404 — no route-enumeration side channel", async () => {
  ...
  it("absent/self Origin + unknown route → 404 (unchanged)", async () => {
  ...
  it("body exactly 1 MiB (1048576 bytes) passes the cap — boundary is > not >= [review item 4]", async () => {
  ...
  it("streaming body over cap without a truthful Content-Length → 413 (exercises the streaming-cancel path, not the Content-Length short-circuit) [review item 4]", async () => {
  ...
  it("class-F listener: Host mismatch → 400", async () => {
    const res = await fetch(classFUrl("/doc/whatever"), { headers: { Host: "evil.com" } });
    expect(res.status).toBe(400);
  });

  it("class-F listener: response carries the class-F CSP header [A3 §5 #1/#2]", async () => {
  ...
  it("class-F listener: unmapped path (no capability logic yet) → 404", async () => {
    const res = await fetch(classFUrl("/doc/whatever"));
    expect(res.status).toBe(404);
  });
  ...
  it("GET / serves the SPA shell: 200, text/html, no Bearer required (navigation)", async () => {
  ...
  it("GET / is served even with a foreign Origin — non-sensitive nav route (A3 §4)", async () => {
    const res = await fetch(apiUrl("/"), { headers: { Origin: "http://evil.example.com" } });
    expect(res.status).toBe(200);
  });

  it("GET /app/bootstrap.js: 200, text/javascript, SPA CSP, no Bearer required", async () => {
  ...
  it("GET /app/../secret (literal traversal, collapsed by the URL parser) → 404, not the daemon's own secrets", async () => {
    const res = await fetch(apiUrl("/app/../secret"));
    expect(res.status).toBe(404);
  });

  it("GET /app/..%2f.. (encoded traversal, allowlist rejects it) → 404", async () => {
    const res = await fetch(apiUrl("/app/..%2f.."));
    expect(res.status).toBe(404);
  });

  it("GET /app/__proto__ (prototype key) → 404, not a 500 — own-keys-only lookup", async () => {
  ...
  it("GET /w/:slug/artifacts (authed-read) with Host mismatch → 400, no body [A3 §5 #7]", async () => {
    const res = await fetch(apiUrl("/w/some-slug/artifacts"), { headers: { Host: "evil.com" } });
    expect(res.status).toBe(400);
    expect(await res.text()).toBe("");
  });

  it("GET /w/:slug/diff with no Bearer → 401, never reaches the unknown-slug 404", async () => {
    const res = await fetch(apiUrl("/w/some-slug/diff?from=a&to=b"));
    expect(res.status).toBe(401);
  });

  it("GET /w/:slug/artifacts with foreign Origin but valid Bearer → 403 (reads reject foreign)", async () => {
  ...
  it("POST /w/:slug/annotations with valid Bearer + self Origin → passes auth, then 404 (empty registry)", async () => {
  ...
  it("POST /w/:slug/annotations with valid Bearer but missing Origin → 403 (state-changing)", async () => {
    const res = await fetch(apiUrl("/w/some-slug/annotations"), {
      method: "POST",
      headers: { Authorization: `Bearer ${TOKEN}`, "Content-Type": "application/json" },
      body: JSON.stringify({ body: "x", intent: "content", target: { quote: { exact: "x" } } }),
  ...
  it("GET /w/:slug/stream (route SHELL) with no Bearer → 401 — the pipeline runs for shells too", async () => {
