...
describe("checkContractVersion", () => {
  ...
  test("exact match (1.14) → ok", () => {
    expect(checkContractVersion("1.14")).toEqual({ status: "ok" });
  });
  ...
  test("minor mismatch, same major (1.15) → stale-minor", () => {
    expect(checkContractVersion("1.15")).toEqual({ status: "stale-minor" });
  });
