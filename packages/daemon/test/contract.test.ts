...
describe("checkContractVersion", () => {
  ...
  test("exact match (1.10) → ok", () => {
    expect(checkContractVersion("1.10")).toEqual({ status: "ok" });
  });

  test("N-1 minor (1.9) → stale-minor", () => {
  ...
  test("minor mismatch, same major (1.11) → stale-minor", () => {
