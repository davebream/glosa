...
describe("checkContractVersion", () => {
  ...
  test("exact match (1.11) → ok", () => {
    expect(checkContractVersion("1.11")).toEqual({ status: "ok" });
  });

  test("N-1 minor (1.10) → stale-minor", () => {
  ...
  test("minor mismatch, same major (1.12) → stale-minor", () => {
