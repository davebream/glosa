...
FIRST VIEWPORT: 1440 wide. Left 232px sidebar: workspace, document outline as § addresses, artifacts, sessions. Top bar: workspace name, session status, More. Artifact bar on the manuscript measure: directory, Read/Review/Edit segmented, History, More; the manuscript face chooser (Default / Serif / Mono) lives inside that More menu as a radio group, never in primary chrome. Centre manuscript at reading measure; § address in the gutter at each heading; marked passages washed in pencil grey or teal with a superscript address; session answers as black in-line reply lines. Right 300px margin: entries by address, pencil or ink, composer under the pencil entry with Discard / Send to session; approval strip pinned at the foot. Provenance line under the manuscript.
...
- 2026-09-15: the face chooser moved from the artifact bar into the pane's More menu (maintainer: a reading preference is a setting, not primary chrome).
- Addresses are shown on margin entries, the composer and gutter-marker names, not as superscripts inside the manuscript: DOM inserted into the rendered content would break the quote-based anchors.
- The session's answer is not printed in line under the passage yet: the journal does not carry the applied text. Session entries are printed in ink in the margin.
