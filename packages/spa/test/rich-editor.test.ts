...
describe("the REQ-8 measurement harness (AC-4) — four metrics over the nine hand-written documents", () => {
  ...
  test("metric 1 — 51 of 799 blocks still cost bytes re-serialized, with no restoration", () => {
    ...
    expect(misses).toBeLessThanOrEqual(51);
    ...
    expect(
      byCause,
      countNote(
        "the per-cause record, a NUMERATOR totalling 51. A move here is not bookkeeping: either the serializer changed, or a document gained a block that is itself lossy. Establish which before touching these numbers.",
      ),
    ).toEqual({
      "link reference definition inlined": 30,
  ...
  test("metrics 2 and 3 — 1 dishonest write of 727; the guard fires on it and, ablated, on 46", () => {
