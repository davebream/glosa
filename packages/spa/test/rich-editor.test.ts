...
const CORPUS_COUNT_NOTE = [
  ...
  "  - metric 1's per-cause map still totals 42,",
...
describe("the REQ-8 measurement harness (AC-4) — four metrics over the nine hand-written documents", () => {
  ...
  // · 40/474 after the roadmap re-sequence: ROADMAP.md's new intro paragraph is one block; every
  //   other change rewrote existing list and paragraph blocks in place. `edits` 422 → 423, numerators
  //   unchanged.
  ...
  //   alpha.19. `edits` 419 → 420, numerators unchanged.
  ...
  //   serializer did not change. The ablated total did NOT move, because a dated heading is not a link
  //   reference definition; that is the alpha.18 case this file's note below describes, not this one.
  // · 40/466 after #144/#153's own documentation: a new `### Added` section under `[Unreleased]` in
  //   CHANGELOG.md is two blocks (the heading and its list), and the README/requirements changes grew
  //   existing blocks rather than adding any. `edits` 416 → 417 with it. The per-cause map, metric 2's
  //   1/1 and metric 3's 0/0 are all unmoved, which is what makes this the bookkeeping case.
  //   IT DID NOT START THERE, and the first attempt is worth recording because it is the failure mode
  //   this harness exists to catch: the new bullets were first written with a blank line between them
  //   and the list below, which makes markdown-it read the WHOLE `### Fixed` list as loose and moved
  //   `ablated` 35 → 36 and the per-cause map's `tight list re-emitted loose` 4 → 5. That is a moved
  //   numerator — a document gaining content that is itself lossy — and the fix was to write the
  //   bullets tight like their neighbours, not to re-baseline the numerator.
  //
  ...
  // shipped/ablated numerators remain unchanged (40, 1/1, 35/35).
  ...
  // 484 → 488 edits.
  // #153 Part 2 documents itself in the corpus it measures: CHANGELOG.md's entry and README.md's
  // watch paragraph are two of the nine, so 542 → 554 blocks / 488 → 500 edits. Metric 1's
  // per-cause map (40), and metric 2/3's shipped (1/1) and ablated (35/35) numerators, held steady
  // across this move as across every earlier one — checked directly against the corpus, not
  // assumed.
  // #159's decisions.md entry and CHANGELOG.md bullet: 554 → 562 blocks / 500 → 508 edits, with the
  // same numerators (40, 1/1, 35/35).
  // Cutting alpha.22: CHANGELOG.md's dated release heading plus a `### Changed` heading and list for
  ...
  // After alpha.23: the README rewritten for the new look and a fresh `[Unreleased]` section, 568 →
  // 578 blocks / 512 → 521 edits. The release case below also applies: `[0.1.0-alpha.23]` gained its
  // link definition, so with `[Unreleased]` back both headings are reference links (cause #1 19 → 20,
  // ablated 35 → 36, shipped still 1).
  // One page with an Edit state: R6 and DESIGN.md rewritten, a decision entry, a `### Changed` list
  // and the README's surfaces table, 578 → 590 blocks / 521 → 532 edits, numerators unchanged.
  // After alpha.24: a fresh `[Unreleased]` with a `### Fixed` heading and list, 590 → 593 blocks /
  // 532 → 534 edits. The release case applies again: with `[Unreleased]` back beside
  // `[0.1.0-alpha.24]`, both headings are reference links (ablated 36 → 37, shipped still 1).
  // The native artifact watch: a decision entry (a heading and six paragraphs, 593 → 600 blocks /
  // 534 → 541 edits); its changelog bullet joins the existing `### Fixed` list. Numerators unchanged.
  ...
  // Re-baselined again for the two-states decision entry: 629 → 638 blocks. The numerators did not
  // move with the denominator — metric 1's per-cause map still totals 42, metric 2 still reports 1
  // shipped dishonest write, metric 3 still 0 missed and 0 false alarms — so the corpus grew and
  // nothing about the serializer changed.
  // Cutting alpha.28: a dated `## [0.1.0-alpha.28]` heading with `### Changed` and `### Fixed`
  // headings and lists, 638 → 643 blocks / 576 → 580 edits. The release case again: the new heading
  // has its definition at the foot of the file, so `link reference definition inlined` 23 → 24 and
  // `ablated` 39 → 40 are its only numerator moves, and `shipped` held at 1.
  ...
  // #229: its own `docs/decisions.md` entry and a CHANGELOG bullet under the existing
  // `[Unreleased]` / `### Fixed` list, 646 → 655 blocks / 582 → 591 edits. No release was cut and
  ...
  // #305/#311's own CHANGELOG bullets — three blocks, no heading, so no reference link either:
  // 669 → 672 blocks / 605 → 608 edits, and every numerator held. Bookkeeping, denominator only.
  // #310: its own `docs/decisions.md` entry — seven blocks — plus a CHANGELOG bullet under the
  // existing `[Unreleased]` / `### Fixed` list and a contract-version edit inside an existing
  // `docs/requirements.md` line, neither of which adds a block: 669 → 676 blocks / 605 → 612 edits
  // measured on its own branch.
  // The draft first moved `soft break inside a code span collapsed` 5 → 6, which is NOT the release
  // shape this file permits, so it was investigated rather than re-baselined: a code span in the new
  // entry straddled a line break. Rewrapping it put every numerator back — metric 1's per-cause map
  // unchanged at 46, 1/1 shipped, 41/41 ablated, 0 missed and 0 false alarms. Denominator only.
  // MERGING #305/#311 AND #310: both branches re-baselined from the same 669/605 base, so neither
  ...
  // 2026-09-26, #371: the "desktop app carries Bun and the CLI" entry in docs/decisions.md and one
  // CHANGELOG bullet, all editable: 775 blocks / 705 edits. Numerators unmoved.
  // Re-measured per-cause 49, shipped 1/1 and ablated 44/44 remain unchanged.
  // #371 release job: README's "Desktop app" subsection, a CONTRIBUTING paragraph and one
  // CHANGELOG bullet, all editable: 780 blocks / 710 edits. Numerators unmoved.
  // 2026-09-26, ad-hoc app distribution: the superseding entry in docs/decisions.md and README's
  // quarantine step, all editable: 790 blocks / 720 edits. Numerators unmoved.
  // 2026-09-26, Homebrew formula: README's brew install paragraph and code block, and its
  // cask-or-formula paragraph, all editable: 793 blocks / 723 edits. Numerators unmoved.
  // v0.1.0-alpha.32 release: one `## [0.1.0-alpha.32]` heading, a reference link whose definition
  // sits at the foot of CHANGELOG.md: 794 blocks / 724 edits, `link reference definition inlined`
  // 28 -> 29 (metric 1: 49 -> 50), `ablated` 44 -> 45, `shipped` still 1/1, no other cause moved.
  // 2026-09-26, the Electron-binary fix: an `### Fixed` heading and one CHANGELOG bullet under
  // `[Unreleased]`: 796 blocks / 725 edits. Numerators unmoved.
  // v0.1.0-alpha.33 release: one `## [0.1.0-alpha.33]` reference-link heading: 797 blocks / 726 edits,
  // `link reference definition inlined` 29 -> 30 (metric 1: 50 -> 51), `ablated` 45 -> 46, `shipped`
  // still 1/1, no other cause moved.
  // 2026-09-26, the x64 baseline-Bun fix: an `### Fixed` heading and one CHANGELOG bullet under
  // `[Unreleased]`: 799 blocks / 727 edits. Numerators unmoved.
  // v0.1.0-alpha.34 release: one `## [0.1.0-alpha.34]` reference-link heading: 800 blocks / 728 edits,
  // `link reference definition inlined` 30 -> 31 (metric 1: 51 -> 52), `ablated` 46 -> 47, `shipped`
  // still 1/1, no other cause moved.
  // 2026-09-26, the App Management install note: an `### Fixed` heading and one CHANGELOG bullet
  // under `[Unreleased]`: 802 blocks / 729 edits. Numerators unmoved.
  // 2026-09-26, Reveal in Finder (#160): an `### Added` heading and one CHANGELOG bullet under
  // `[Unreleased]`: 804 blocks / 730 edits. Numerators unmoved.
  const BLOCKS = 804;
  ...
  test("metric 1 — 52 of 804 blocks still cost bytes re-serialized, with no restoration", () => {
    ...
    expect(misses).toBeLessThanOrEqual(52);
    ...
    // (39 → 40), plus `## [0.1.0-alpha.23]`, which alpha.23 gave a link definition (40 → 41), plus `## [0.1.0-alpha.24]`, which alpha.24 gave a link definition (41 → 42), plus #270's own `## [Unreleased]` heading, whose definition was already at the foot of the file (42 → 43), plus #271's, for the same reason (43 → 44), plus `## [0.1.0-alpha.28]`, which alpha.28 gave a link definition (44 → 45), plus the `## [Unreleased]` heading opened for the next release, whose definition is already at the foot of the file (45 → 46), plus `## [0.1.0-alpha.29]`, which alpha.29 gave a link definition (46 → 47) — the release case named below, not a serializer change. #146's session-registration boundary then grew the corpus to 722/656 and alpha.29's release prep folded that entry into an existing section again, returning it to 720/655. Both moves are documentation only; no numerator moved in either direction. A genuine
    ...
    expect(
      byCause,
      countNote(
        "the per-cause record, a NUMERATOR totalling 52. A move here is not bookkeeping: either the serializer changed, or a document gained a block that is itself lossy. Establish which before touching these numbers.",
      ),
    ).toEqual({
      "link reference definition inlined": 31,
  ...
  test("metrics 2 and 3 — 1 dishonest write of 730; the guard fires on it and, ablated, on 47", () => {
    ...
    expect(
      ...
      // describes a tree containing both sets of documentation. The alpha.29 release then moved
      // `ablated` 41 → 42 for the reason stated above — its `## [0.1.0-alpha.29]` heading gained a
...
describe("the per-node-type opt-out — nothing outside the modelled inventory is ever rewritten", () => {
  ...
  test("the inventory is the editor schema's own, minus the deliberately opaque, in both directions", () => {
    // Held against `editorSchema` — the schema `parseMarkdown()` ACTUALLY produces — minus the two
    // types this file deliberately refuses to model. A name the schema has and the list lacks would
    // drop ordinary documents onto the raw path silently; a name in the list the schema does not
    // have is dead weight that would tell the next reader the wrong thing. Neither is visible from
    // any other test here.
    //
    // This used to compare against `markdownSchema`, the vendored CommonMark schema, on the grounds
    // that it does not move. It moved: #270 gave the editor table nodes and a strikethrough mark so
    // it can represent everything the daemon's renderer shows a reader, and those live on
    // `editorSchema` only. Comparing against the schema in use is also the stronger check — it is
    // the one that would catch a node added to the editor and forgotten here.
    const opaqueNodes = ["glosa_raw"];
    const opaqueMarks = ["glosa_comment"];
    expect([...MODELLED_NODE_TYPES].sort()).toEqual(
      Object.keys(editorSchema.nodes)
        .filter((name) => !opaqueNodes.includes(name))
        .sort(),
    );
    expect([...MODELLED_MARK_TYPES].sort()).toEqual(
      Object.keys(editorSchema.marks)
        .filter((name) => !opaqueMarks.includes(name))
        .sort(),
    );
    // The opaque types exist and are excluded on purpose, not missing by accident: if one were
    // renamed, the filters above would silently stop excluding anything and this test would still
    // pass while the inventory quietly gained a type it must never model.
    for (const name of opaqueNodes) expect(Object.keys(editorSchema.nodes)).toContain(name);
    for (const name of opaqueMarks) expect(Object.keys(editorSchema.marks)).toContain(name);
  });
});
