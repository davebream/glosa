...
describe("createDataAccess — request shape", () => {
  ...
  test("the removed wiring/init routes are gone from the data-access surface (#152)", () => {
    const da = createDataAccess({ fetchFn: async () => jsonResponse(200, {}), storage: fakeStorage({}) });
    expect("getWiringStatus" in da).toBe(false);
    expect("triggerInit" in da).toBe(false);
  });
