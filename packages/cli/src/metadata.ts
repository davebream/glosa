...
export async function runMetadata(
  ...
  if (!new Set(["set", "show", "clear"]).has(args.action)) {
    return {
      ...
      data: {},
      warnings: [],
      error: { code: "usage", kind: "usage", message: `unsupported metadata action '${args.action}'` },
    };
  }
  if (args.action === "set" && !args.file) {
    return {
      ...
      data: {},
      warnings: [],
      error: { code: "usage", kind: "usage", message: "metadata set requires a descriptor file" },
    };
  }
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          kind: "usage",
          message: `could not read descriptor JSON: ${(error as Error).message}`,
        },
      };
    }
  }
  ...
  try {
    ...
    if (args.action === "show") {
      ...
      if (!metadata) {
        return {
          ...
          data: {},
          warnings: [],
          error: { code: "metadata-not-found", kind: "not_found", message: "workspace metadata is not registered" },
        };
      }
    ...
    if (isApiError(error)) {
      return {
        ok: false,
        command,
        exitCode: error.status === 409 ? EXIT_CODES.ENTRY_ERROR : EXIT_CODES.USAGE,
        data: {},
        warnings: [],
        error: {
          code: error.status === 409 ? "metadata-conflict" : "invalid-metadata",
          kind: error.status === 409 ? "conflict" : "validation",
          message: error.problem?.title ?? error.message,
        },
      };
    }
    return { ...daemonUnreachableEnvelope(command, (error as Error).message), data: {} };
...
export function printMetadataResult(result: CommandEnvelope<MetadataData>, json: boolean): void {
  ...
  if (result.data.metadata) {
    process.stdout.write(
      `glosa metadata: ${result.data.metadata.id} (${result.data.metadata.artifacts.length} artifacts)\n`,
    );
  } else {
