...
CLI/hooks call the daemon HTTP API (mutation under an in-process async mutex keyed by workspace
canonical path); if daemon unreachable they FAIL LOUDLY, never do unsynchronized writes. Reuses this
...
- Files: `.glosa/journal.ndjson` (append-only), `.glosa/inbox/<id>.json` (immutable, temp→fsync→rename), `.glosa/journal.quarantine.ndjson`.
...
- Every git call = argv array (never shell), `--git-dir=<abs>/.glosa/shadow.git --work-tree=<abs-root>`, env `GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_SYSTEM=/dev/null GIT_TERMINAL_PROMPT=0`, constant identity `glosa <glosa@localhost>` (attribution in commit TRAILERS not author), `--` before every pathspec (+ `./` prefix for paths starting `-`).
...
## F20 — one canonical matcher
- chokidar v4 dropped globs, git-pathspec ≠ minimatch → NO consumer holds its own glob. Canonical = **picomatch** (zero-dep, fs-free). One `resolveMatchedFiles(root) → {tracked, oversize}` → explicit normalized LIST. Chokidar watches DIRECTORIES, raw events filtered through the set; sidebar renders tracked; git stages tracked ∪ previously-tracked. Same list, 3 consumers, zero drift.
...
- Config: `{artifacts:{include:["**/*.md","**/*.html","**/*.txt"], exclude:[".glosa/**","**/node_modules/**",".*/**"], maxFileBytes:2097152, followSymlinks:false}}`.
