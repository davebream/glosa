...
reports `connected:false` — including for an unknown session id, which is treated as free. Homebrew/npm
Codex installs do not provide a managed daemon; users either install the standalone distribution or
separately run:
