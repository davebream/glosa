...
- delivery_attempt.detail: `{via:monitor|codex_app_server|mcp_pull, session, outcome:attempted|transport_accepted|presented|failed, reason:initial|re_nudge, error?}`. The provider-neutral session stream is the monitor and Codex push rail; MCP pull is the only other transport. `channel`, `asyncRewake`, `gate`, `stop` and `userprompt` are retired (#152) — a journal written before that still replays (an unknown `via` is carried, never fatal), but no writer emits them and the drain route refuses them.
...
- `external_edit` (R3, #153) is a `common` entry and inherits that table unchanged — `dismissed` is how a person closes one, and no new kind or terminal set exists for it. It is excluded from `prepare`'s candidate set at `eligibleDeliveryEntriesLocked`, the single gate feeding both `previewDelivery` and `prepareDelivery`, so no delivery route ever offers one and no `delivery_attempt` is recorded against it; it is still retrievable through `glosa inbox get`/MCP, whose presentation states the checkpoint pair, the observation source, and that attribution is `unknown` and nothing is being asked of the reader. Its bounded-hunk/continuation-cursor treatment is the human-edit one, since the truncation contract is about size rather than kind. No attention fold needs an exclusion: every one of them filters `kind === "attention"`, and this entry's kind is `common`.
