...
- delivery_attempt.detail: `{via:monitor|codex_app_server|channel|asyncRewake|gate|stop|userprompt|mcp_pull, session, outcome:attempted|transport_accepted|presented|failed, reason:initial|re_nudge, error?}`. The provider-neutral session stream is the monitor and Codex push rail; `channel` and the hook values remain only for the #151→#152 migration window.
