...
- **A client only ever stops a daemon its own install started.** Two installs on one machine (a source checkout beside a release, two globals) otherwise evict each other on every command. The two restart paths carry opposite burdens of proof: an upgrade (client strictly newer) restarts unless the daemon is PROVABLY foreign, so a daemon predating `install_id` still upgrades; equal semver with a different hash restarts only when the daemon is PROVABLY ours, so an unknown identity fails closed.
- `ensureDaemon()` verifies the lock/handshake pair, then applies this matrix: higher client semver → SIGTERM+replace regardless of protocol, unless `install_id` proves a different install → FAIL; lower client semver → reuse only when protocol-compatible, otherwise FAIL `incompatible glosa versions installed` without signalling; equal semver+hash → reuse only when protocol-compatible; equal semver+different hash → SIGTERM+replace only when `install_id` matches, otherwise FAIL naming the PID and the `kill -TERM` recovery; missing legacy identity → replace. Malformed identity or any lock/handshake disagreement fails closed without signalling.
- A **source checkout** (detected by the presence of `packages/daemon/test`, which the npm tarball never ships) defaults `GLOSA_HOME` to `~/.glosa-dev/<install_id>` and `GLOSA_PORT` to a deterministic even port in 60000–65498, so it shares neither state nor port with a published install. The home sits outside the working tree: the pairing token must not land where repo-wide agent reads, backups and IDE indexers look. An explicit `GLOSA_HOME`/`GLOSA_PORT`/`--port` always wins, and the CLI says once, on an interactive terminal only, which values it derived.
...
- Replacement signals only the verified PID, waits ≤10s (bounded by the discovery deadline) for that exact lock instance to disappear/change or that PID to exit, then re-enters the ordinary read-or-spawn loop. Concurrent clients therefore converge through bind/O_EXCL/CAS; clients never unlink a replacement lock or spawn independently. If no live daemon exists, spawn = detachedSpawn + poll handshake ≤5s; timeout→FAIL w/ daemon.log path.
- **The socket listener** (`<GLOSA_HOME>/run/api.sock`, A3 §3.2) is bound LAST, after the class-F
  bind, for the same reason class-F follows the lock: the TCP bind plus the O_EXCL CAS is what
  decides which process is the daemon, and nothing may widen the window between them. Order is
  `mkdir run/ 0700` → `Bun.serve({unix})` → `chmod 0600` → serve; `Bun.serve` creates the socket
  0755, so the directory's traversal bit is what covers the instant before the chmod. `EADDRINUSE`
  does not distinguish a live owner from a dead one: a bind onto a live socket throws it, and so
  does a bind over the inode a SIGKILLed daemon left behind, which stays refused until something
  unlinks it. So the bind probes the path before reclaiming, on the same evidence rule as
  `reclaimStaleLock` — only a clean `ECONNREFUSED` proves nobody is listening and authorizes the
  unlink; a path that still accepts, or any ambiguous result, aborts the boot. Retrying blindly
  would start a second daemon beside a live one; refusing outright would let a single crash brick
  every later boot. Any failure here — including
  a run dir that cannot be made 0700, or a chmod that cannot be confirmed — releases the lock,
  stops the already-bound listeners and exits 3, because a daemon whose socket permissions are
...
- Event vocab (writer / changes-state / idem-key): created(daemon/→pending/entry); delivery_attempt(daemon/NO/(entry,attempt_seq)); delivered(daemon/pending→delivered/(entry,delivered)); seen(daemon|human/delivered→seen [attention]/(entry,seen)); resolved(session:<id>|human/→applied|rejected|stale/(entry,terminal) first-terminal-wins); dismissed(human/→dismissed/(entry,terminal) first-terminal-wins; a live claim over the entry is released `by:"human"` first — the human wins, A4 §F05); done(human|session/seen→done [attention]); staled(daemon/→stale); expired(daemon|session:<id>/→expired [attention]; a session's own withdrawal of its cancelled question carries detail.withdrawn:true). A seen/response mutation may append missing `delivered` and `seen` transitions under the same workspace mutex; every completion therefore has the auditable `seen→done` path. Claim events (issue #155, A4 §F05) — claim_taken(session:<id>/NO), claim_renewed(session:<id>/NO), claim_released(session:<id>|human/NO), claim_expired(daemon/NO) — never change an entry's status: a claim is a fence around who may prove an interval, not a lifecycle state.
- delivery_attempt.detail: `{via:monitor|codex_app_server|mcp_pull|watch, session, outcome:attempted|transport_accepted|presented|failed, reason:initial|re_nudge, error?}`. The provider-neutral session stream is the monitor and Codex push rail; MCP pull is the only other ordinary transport. `channel`, `asyncRewake`, `gate`, `stop` and `userprompt` are retired (#152) — a journal written before that still replays (an unknown `via` is carried, never fatal), but no writer emits them and the drain route refuses them. `via:"watch"` (issue #153 Part 2) is the one member of this vocabulary that never appears on anything but an `external_edit` entry, and only for the session whose `glosa_watch`/`GET /w/:slug/watch` call it was recorded against — see F23's `external_edit` bullet below.
...
- An unbound session's composite drain resolves its candidate workspaces from `SessionRegistry.forWorkspace`'s R2 predicate, run per workspace against that ONE session id. Ordinarily that predicate reads the session's live registry row; a request carrying an explicit scope (issue #205 — the MCP `mcp_pull` generic path only) instead substitutes that scope for the row's `cwd` throughout the predicate, for this session id alone, every OTHER session's row still read live. Captured once at the route, before the composite mutex is even acquired, and reused by both the ordinary candidate filter and the `record.cwd` self-heal fallback — no code path inside a scoped drain re-reads the row for its OWN routing decision, so a registration/bind that moves the row after the drain is admitted cannot redirect it. This is a request-scoped override, not a registry mutation: the row itself is untouched, and every other consumer of `forWorkspace` (delivery routing, attention counts, `glosa forget`'s liveness check) is unaffected.
- An admitted scoped drain survives the row's LOSS as well as its redirection (issue #205, A10). `forWorkspace`'s liveness filter runs before the scope override is applied, so a session that deregisters or whose lease expires between admission (the route capturing `record` and validating `scope`) and selection would otherwise have no live row left to override, silently selecting nothing — a second, later-discovered form of the same defect: the row can no longer redirect an admitted drain, but was still able to suppress it. The override now carries the ALREADY-captured `record` snapshot independently of the live registry list, injected as an extra candidate only when no live row for that session id remains (a live row under the same id, however stale-seeming to a caller, still wins — this exists to survive loss, not to shadow a fresh re-registration, which A3 above already covers). The drain therefore completes on its captured scope regardless of what happens to the row afterward. Its composite acknowledgement must then also not require that row: `POST /api/sessions/:id/deliveries/:deliveryId/ack`'s composite-token branch (§5.15) moved ahead of the route's row requirement, since a `CompositeDeliveryRegistry` reservation already authenticates its own session match (`reservation.session !== session` → `"missing"`) and does not depend on the registry row at all. Ordinary, non-composite acknowledgement is unchanged and still requires the row, to resolve which bus to acknowledge against. Without this second half, a completed drain's acknowledgement would 404 and its entries would sit reserved until the composite registry's own 30-second TTL/lazy pruning released them — a silent, temporary drop, not a redirect, but still a violation of "the drain completes."
- A claim is a fence, not a filter (issue #155): drain keeps presenting a claimed entry, and every presentation carries `claims` — the live claims on the entry or its file, exclusive first, at most four, with `truncation.omitted_claims` for the rest — plus one text line per claim after the body, so a second session sees who is already working on it before it tries.
- Signals (A1 §5.11g) are a separate axis from this lifecycle. They are derived from claim events in memory, addressed to one session each, and never change an entry's status or append to the journal. A drain carries the session's unacknowledged signals in `signals[]`, outside the entry and batch caps, with their own cap of eight signals and 8 KiB.
- Limits are measured with `TextEncoder`: 16 KiB per serialized entry, 32 KiB per serialized batch,
  maximum eight entries. The claims are reserved INSIDE the entry budget before the body is sized: body hunks yield, claims never do. An annotation always retains id/kind/artifact/body-or-prefix/intent/quote/
...
- `external_edit` (R3, #153) is a `common` entry and inherits that table unchanged — `dismissed` is how a person closes one, and no new kind or terminal set exists for it. It is excluded from `prepare`'s candidate set at `eligibleDeliveryEntriesLocked`, the single gate feeding both `previewDelivery` and `prepareDelivery`, so no ORDINARY delivery route (monitor, Codex push, MCP pull) ever offers one; it is still retrievable through `glosa inbox get`/MCP, whose presentation states the checkpoint pair, the observation source, and that attribution is `unknown`. Its bounded-hunk/continuation-cursor treatment is the human-edit one, since the truncation contract is about size rather than kind. No attention fold needs an exclusion: every one of them filters `kind === "attention"`, and this entry's kind is `common`.
- **`external_edit` and `via:"watch"` (issue #153 Part 2).** The exclusion in the bullet above is now conditional rather than absolute: `glosa_watch`/`GET /w/:slug/watch` is a SEPARATE selection from `eligibleDeliveryEntriesLocked` (that gate is untouched and still excludes the kind from every ordinary route) that records `delivery_attempt{via:"watch", session, outcome:"transport_accepted"}` then `{outcome:"presented"|"failed"}` for exactly the session that called it, never a status transition. A watch's cursor is "in-scope, non-terminal `external_edit` not yet `presented` via watch **to this session**" — a `presented` mark from session A never suppresses the entry for session B, unlike every other `via`'s global `presented`-suppression at `eligibleDeliveryEntriesLocked` (which does not apply here in the first place, since the kind never reaches that gate). The watch presentation text says plainly the session is seeing it because it explicitly asked to watch, replacing the "nothing is being asked" wording ordinary retrieval uses. No consumer of `deliveryAttempts` that isn't watch-aware changes behavior: badge/retention counts read status only, GC reads status only, and adoption copies attempts verbatim regardless of `via`.
- The journal-derived pending count is TWO signals, named separately (#153): a BADGE-facing count (`GET /api/status`'s workspace row, read by the SPA badge and `doctor`'s pending-delivery check) that excludes `external_edit`, and a RETENTION-facing count (`WorkspaceIndex`'s GC hard-remove guard and the orphaned-home-state scanner) that does not. Counting a non-deliverable entry as "N queued" would promise an ordinary push delivery that never comes — since Part 2 a bound session can pull its own through `GET /w/:slug/watch`, which is the one opt-in exception and changes neither count; excluding it from retention would tell GC and the stranded-work scanner that a workspace holds nothing, which is the deletion this promise ("an undismissed one stays pending until dismissed") exists to prevent.
- Common entries terminal: applied/rejected/stale/dismissed. Attention terminal: done/expired/stale. Event with guard `from`≠current → ignored on replay (idempotent). The fold records `terminalBy`, the `by` of the transition that made the entry terminal (only on the winning side of the guard). A second resolve on a terminal entry is refused under the mutex BEFORE any checkpoint: the SAME session with the SAME outcome gets a replay of its original result (`replayed:true`, nothing appended); anyone else gets `409 entry-resolved{terminal_by,entry_status}`. Dismiss, defer and withdraw share that guard without the replay (`by:"human"` names every person, so a repeat is not provably a retry). No journal field `idem` participates: the decision is the fold's, per session, and the loser never takes a checkpoint. Ordinary `done.detail` is `{outcome:"done"|"approved"|"changes_requested", response?:string}`; review actions accept the two review outcomes, while generic actions accept `done`. Approval-mode attention stores exactly `{outcome:"approved",target_path,revision_id,completed_at}` after validating the current `source_sha256`. Creation is serialized with an atomic non-terminal uniqueness check on workspace+target_path, using journal-derived `entry_created.detail.approval_mode`/`target_path` when present and a fail-closed immutable-inbox fallback for legacy events without those additive fields. A terminal retry returns the original detail without appending.
