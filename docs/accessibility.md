...
| Preview / Annotate / Edit | Tab reaches every mode; Command/Ctrl+1–3 switches the active pane's mode; focus remains on the active mode after its DOM refresh. Only the focused pane exposes a mode control, so several open artifacts never present several switchers to the same keyboard. |
