...
- ⬜ **P4.5 jethro adapter** — recognize plugin-data path, session binding from `session_history`, stage ordering, derived-from edge, class-F manifest resolution. _Appendix: requirements R7 · Accept: against fixture copies of the real session + a synthetic post-#314 state + speech-notes fixture · CC: no_
...
- Do Phase 1 → 5 in order; within a phase, respect intra-task deps. A CC task is not done until its adversarial-review + fault tests pass.
