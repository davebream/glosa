...
1. **The first adapter cannot satisfy the core routing rule.** R2 says an artifact routes only to a live session whose `cwd` is its nearest ancestor. Real integration workspaces and artifacts can live in tool-owned data outside the Claude process's project cwd. An existing SessionStart hook already records the Claude `session_id` and exact `transcript_path` into active workspace state, proving that an explicit association is necessary. The PRD neither consumes that association nor permits a provider-supplied workspace binding. In the real topology, feedback can park forever even while the correct session is alive.
...
The first specialized adapter recognizes external workspace state, orders artifacts by stage, identifies the canonical source, associates derived HTML with its source manifest, and routes comments on transformed output as pipeline feedback rather than pretending every selection maps to literal source text. A companion renderer patch is expected to add source ranges and preserve chunk identity into rendered HTML.
...
That conceptual decomposition is good. The durable file state is separated from lossy notification transports, the daemon is separated from per-host shims, and integration-specific interpretation is intended to remain outside the core.
...
- The final PRD follows the late “no live-workflow experiments” correction: Plannotator is not shipped or trialed, and release waits for an offline rehearsal.
...
- Class R and class F viewers, manifest-first derived-HTML provenance, transformed-content intent routing, and chunk threading follow the later anchoring findings.
...
- The class-F fixture paths, canonical-source fallback, and external-renderer title bug are carried into tasks.
...
| Validation cadence | Options §6 and decisions accepted item 9 use per-project milestones and a legitimate stop after M3. | The late amendment and PRD require full v1 before live use. | The PRD follows the latest decision, but old “success after 3–4 projects” and stop-point language should be marked superseded. |
| Workspace/worktree | Early options place `.desk` in a writing-session directory and use `artifacts/` as the Git work-tree. | R1 makes any directory a workspace and uses the whole workspace with artifact globs. | This is a substantial generalization, not a rename. Initialization, baseline, matching, and session association need to be re-specified for the broader model. |
...
| Package layout | Decisions line 123 suggests `packages/daemon / spa / adapters-domain`. | Requirements line 63 specifies `packages/adapters/domain`. | Minor naming drift, but build scripts and package names should use one canonical layout. |
| Privacy | Options lines 382 and decisions line 93 note that current derived documents are deployed publicly. | Requirements NFR promises no external runtime calls/uploads and class F runs document JS untouched. | Glosa can remain local, but foreign HTML may itself make network requests. The runtime privacy promise needs a browser-level policy, not just a daemon policy. |

The largest hidden contradiction is between the external integration's real storage model and R2. The background options show a registered Claude cwd in the producer repository, while artifact data is under a separate tool-owned workspace. Existing integration code confirms this is not hypothetical: its SessionStart hook reads `session_id` and `transcript_path`, finds the active writing session, and records the association there. R2 nevertheless makes “cwd is an ancestor of the artifact” authoritative. The adapter needs an explicit provider-supplied association that is at least as authoritative as cwd containment.
...
The core attribution claim is not supportable from chokidar events. The watcher sees bytes changed; it does not know which process or person wrote them. Agent writes unrelated to an inbox entry, editor formatters, an external-renderer rerender, and human saves all look alike. Likewise, glosa cannot checkpoint “before an agent applies an entry” unless the agent first invokes a glosa begin/apply operation. `glosa resolve` happens after the edit.
...
| **R2 — session registry** | **Blocking for the external integration and transcript viewer.** | The hook has no documented Claude PID; registry omits `transcript_path`; `most-recently-active` is used in R3 but no `last_active_at` field or update source exists; direct concurrent JSON writes can lose sessions; cwd-ancestor routing cannot route tool-owned artifacts; no provider binding/lease schema; no definition for nested workspaces or a session changing cwd. | Define stale lease threshold, process-permission errors, duplicate SessionStart, `/clear` and `/resume` transitions, picker persistence, and whether a hint may name a session whose cwd is not an ancestor. |
...
| **R7 — external adapter** | **Cannot be correct under R2.** | No adapter interface/schema despite adapter boundary being load-bearing; recognition root is unclear because state lives in tool-owned data; no explicit integration-session ↔ Claude-session association; state schema is only an absolute external code reference; stage ordering mapping absent; manifest version/schema absent; pipeline-feedback output contract absent; stale-render algorithm absent; T6 assumes companion fields before T7 creates them. | Define behavior for malformed/null canonical pointer, multiple active projects, old sidecars, output selection among timestamps, a missing external renderer, and adapter version compatibility. |
...
1. Claude runs from the external producer's project cwd.
2. The active writing project and derived HTML live under tool-owned data or an output path outside that cwd.
...
5. A later SessionStart in the producer repo still does not have the writing workspace as its cwd, so the parked entry never drains.
...
- Starting Claude in a copied artifact folder does not prove the real out-of-cwd adapter routing.
...
- A provider-specific workspace/session association for the external integration.
...
7. **Actual cross-directory topology suite:** Claude cwd is the producer repo while the writing workspace is tool-owned data; provider binding routes the correct entry and transcript.
...
|---|---|---|---|---|---|
| **F01** | **BLOCKING** | Analysis / correctness | Requirements R2 lines 100–103; R7 lines 218–234; glossary lines 339–349 | “A file resolves to the live session whose cwd is the nearest ancestor” cannot route real artifacts stored in tool-owned data outside the producer repo cwd. The existing integration hook records the necessary Claude-session ↔ writing-session association. | Add a versioned provider binding `{provider, session_id, workspace_path, transcript_path, lease, evidence}`. Make an explicit live provider binding authoritative before cwd fallback. Add an actual cross-directory-topology acceptance test. |
...
| **F14** | **BLOCKING** | Risk / release | T8 lines 303–315 | The hard release gate is nondeterministic, machine-specific, and uses a topology that can bypass the cross-directory routing defect. It omits crash, concurrency, security, and format-evolution failures. | Make deterministic fake-agent/fault/security suites mandatory. Keep a version-pinned manual real-Claude rehearsal as an additional compatibility report, using the actual producer-cwd/tool-owned-workspace topology and Channels-on/off runs. |
